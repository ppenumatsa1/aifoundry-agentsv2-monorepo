...
from invoice_assistant.evals.batch import build_project_client

DEFAULT_DATA_PATH = (
    Path(__file__).resolve().parents[1]
    / "src/invoice_assistant/evals/datasets/golden_capture.jsonl"
)
...
def load_judge_model_config() -> dict:
    ...
    deployment = os.getenv("AZURE_AI_MODEL_DEPLOYMENT_NAME") or os.getenv("AZURE_OPENAI_MODEL")

    if not deployment:
        raise SystemExit("AZURE_AI_MODEL_DEPLOYMENT_NAME (or AZURE_OPENAI_MODEL) is required.")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--evaluation-name",
        default="invoice-assistant-agent-eval",
